	...
	{	 
            ...
            return new Dictionary<String, ForeignField>() 
			{
				...
				},
				{
					...
					new ForeignField() {                        
						Property = "Expedient", 
                        TableAlias = "EX", 
                        Column = nHManager.Instance.GetTableColumn(typeof(moleQule.Store.Data.SupplierRecord), "Codigo")
					}
				}
			};
        }
		...
		internal static string JOIN(QueryConditions conditions)
		{
			...
			if (conditions.OutputDelivery != null)
			{
				query += @"
					INNER JOIN " + ws + @" AS WS ON WS.""OID_WORK_REPORT"" = WR.""OID"" 
						AND WS.""ENTITY_TYPE"" = " + (long)moleQule.Common.Structs.ETipoEntidad.OutputDelivery + @"
			...
			if (conditions.Acreedor != null)
			{
				query += @"
					INNER JOIN " + ws + @" AS WS ON WS.""OID_WORK_REPORT"" = WR.""OID"" 
						AND WS.""ENTITY_TYPE"" = " + (long)moleQule.Common.Structs.ETipoEntidad.Empleado + @"
			...
			if (conditions.Tool != null)
			{
				query += @"
					INNER JOIN " + ws + @" AS WS ON WS.""OID_WORK_REPORT"" = WR.""OID"" 
						AND WS.""ENTITY_TYPE"" = " + (long)moleQule.Common.Structs.ETipoEntidad.Tool + @"
	    ...
	    internal static string SELECT(QueryConditions conditions, bool lockTable)
        {
			...
			query += Common.EntityBase.LOCK("WR", lockTable);
