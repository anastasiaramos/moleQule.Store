...
namespace moleQule.Library.Store
{
	...
	{	 
		...
		public moleQule.Common.Structs.ETipoEntidad EEntityType { get { return (ETipoEntidad)_record.EntityType; } }
		public string EntityTypeLabel { get { return moleQule.Common.Structs.EnumText<ETipoEntidad>.GetLabel(EEntityType); } }
	...
	{	 
		...
		public moleQule.Common.Structs.ETipoEntidad EEntityType { get { return _base.EEntityType; } set { EntityType = (long)value; } }
		...
		internal void DeleteSelf(WorkReport parent)
		{
            ...
            if (EEntityType == moleQule.Common.Structs.ETipoEntidad.OutputDelivery)
            {
                ...
                type.InvokeMember("Delete"
                                    ...
                                    , null, null, new object[3] { OidResource, ETipoEntidad.WorkReport, parent.SessionCode });
		...
		internal static string JOIN_RESOURCE(ETipoEntidad entityType, QueryConditions conditions)
		{
            ...
            switch (entityType)
			{
				case moleQule.Common.Structs.ETipoEntidad.OutputDelivery:
					{
						string rs = nHManager.Instance.GetSQLTable(typeof(moleQule.Invoice.Data.OutputDeliveryRecord));
						query += @"
				...
				case moleQule.Common.Structs.ETipoEntidad.Empleado:
					{
                        string rs = nHManager.Instance.GetSQLTable(typeof(moleQule.Store.Data.EmployeeRecord));
						query += @"
						INNER JOIN " + rs + @" AS RS ON RS.""OID"" = WO.""OID_RESOURCE"" AND WO.""ENTITY_TYPE"" = " + (long)entityType;
					}
					break;
				...
				case moleQule.Common.Structs.ETipoEntidad.Tool:
					{
                        string rs = nHManager.Instance.GetSQLTable(typeof(moleQule.Store.Data.ToolRecord));
						query += @"
						INNER JOIN " + rs + @" AS RS ON RS.""OID"" = WO.""OID_RESOURCE"" AND WO.""ENTITY_TYPE"" = " + (long)entityType;
					}
					break;
		...
		internal static string WHERE(QueryConditions conditions)
		{
			...
			if (conditions.EntityType != moleQule.Common.Structs.ETipoEntidad.Todos)
				query += @"
	    ...
	    internal static string SELECT(QueryConditions conditions, bool lockTable)
        {
			...
			else
			{
				switch (conditions.EntityType)
				{
					...
					case moleQule.Common.Structs.ETipoEntidad.Todos:
					default:
		...
		{
			conditions.EntityType = moleQule.Common.Structs.ETipoEntidad.OutputDelivery;

			conditions.ExtraJoin = JOIN_RESOURCE(moleQule.Common.Structs.ETipoEntidad.OutputDelivery, conditions);
		...
		{
			conditions.EntityType = moleQule.Common.Structs.ETipoEntidad.Empleado;

			conditions.ExtraJoin = JOIN_RESOURCE(moleQule.Common.Structs.ETipoEntidad.Empleado, conditions);
		...
		{
			conditions.EntityType = moleQule.Common.Structs.ETipoEntidad.Tool;

			conditions.ExtraJoin = JOIN_RESOURCE(moleQule.Common.Structs.ETipoEntidad.Tool, conditions);
