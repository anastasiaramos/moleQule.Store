...
namespace moleQule.Library.Store
{
	...
	/// </summary>
	[Serializable()]
	public class NavieraInfo : ReadOnlyBaseEx<NavieraInfo>, IAcreedorInfo, IAgenteHipatia, ITitular
	{
		#region IUser
		...
		public Type TipoEntidad { get { return typeof(Naviera); } }
		...
		protected ShippingCompanyBase _base = new ShippingCompanyBase();

		protected PrecioTrayectoList _precio_trayectos = null;
		...
		public PrecioTrayectoList PrecioTrayectos { get { return _precio_trayectos; } }
        ...
        public void CopyFrom(Naviera source) { CopyValues(source); }

        public decimal GetPrecioPuertoOrigen(string origen, string destino)
        {
            foreach (PrecioTrayectoInfo item in PrecioTrayectos)
            {
                if ((item.PuertoOrigen == origen) && (item.PuertoDestino == destino))
                    return item.Precio;
            }
        ...
        /// </remarks>
		protected NavieraInfo() { /* require use of factory methods */ }
		private NavieraInfo(int sessionCode, IDataReader reader, bool retrieve_childs)
		{
			SessionCode = sessionCode;
			Childs = retrieve_childs;
			Fetch(reader);
		}
		internal NavieraInfo(Naviera item, bool copy_childs)
		{
			CopyValues(item);
			...
			if (copy_childs)
			{
				...
				_precio_trayectos = (item.PrecioTrayectos != null) ? PrecioTrayectoList.GetChildList(item.PrecioTrayectos) : null;				
			}
		}
		...
		/// <remarks/>
		public static NavieraInfo GetChild(int sessionCode, IDataReader reader) { return GetChild(sessionCode, reader, false); }
		public static NavieraInfo GetChild(int sessionCode, IDataReader reader, bool childs)
        {
			return new NavieraInfo(sessionCode, reader, childs);
		}
        ...
        /// <returns>Objeto <see cref="ReadOnlyBaseEx"/> construido a partir del registro</returns>
        public static NavieraInfo Get(long oid)
        {
        ...
        /// <returns>Objeto <see cref="ReadOnlyBaseEx"/> construido a partir del registro</returns>
		public static NavieraInfo Get(long oid, bool retrieve_childs)
		{
			CriteriaEx criteria = Naviera.GetCriteria(Naviera.OpenSession());
			criteria.Childs = retrieve_childs;

			if (nHManager.Instance.UseDirectSQL)
				criteria.Query = NavieraInfo.SELECT(oid);
			else
				criteria.AddOidSearch(oid);
	
			NavieraInfo obj = DataPortal.Fetch<NavieraInfo>(criteria);
			Naviera.CloseSession(criteria.SessionCode);
			return obj;
		}
			...
			try
			{
				if (nHMng.UseDirectSQL)
				{
                    ...
                    if (Childs)
					{
						...
						query = PrecioTrayectoList.SELECT(this);
                        reader = nHMng.SQLNativeSelect(query, Session());
                        _precio_trayectos = PrecioTrayectoList.GetChildList(reader);						
                    }
				}
			}
			catch (Exception ex)
        ...
        public static string SELECT(long oid) { return Naviera.SELECT(oid, false); }
