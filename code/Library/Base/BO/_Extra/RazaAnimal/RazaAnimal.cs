...
{
	[Serializable()]
	public class RazaAnimalRecord : RecordBase
	{
		#region Attributes
		...
		public virtual void CopyValues(RazaAnimalRecord source)
		{
			if (source == null) return;
	...
	[Serializable()]
	public class RazaAnimalBase
	{
		#region Attributes

		private RazaAnimalRecord _record = new RazaAnimalRecord();

		#endregion

		#region Properties

		public RazaAnimalRecord Record { get { return _record; } }
		...
		internal void CopyValues(RazaAnimal source)
		{
			if (source == null) return;

			_record.CopyValues(source.Base.Record);
		}
		internal void CopyValues(RazaAnimalInfo source)
		{
			if (source == null) return;
	...
	/// </summary>
    [Serializable()]
	public class RazaAnimal : BusinessBaseEx<RazaAnimal>
	{
		#region Attributes
		...
		public virtual RazaAnimal CloneAsNew()
		{
			RazaAnimal clon = base.Clone();

			//Se definen el Oid y el Coidgo como nueva entidad
			
			clon.Base.Record.Oid = (long)(new Random()).Next();

			clon.SessionCode = RazaAnimal.OpenSession();
			RazaAnimal.BeginTransaction(clon.SessionCode);
		...
		protected virtual void CopyFrom(RazaAnimalInfo source)
		{
			if (source == null) return;
		...
		private RazaAnimal(RazaAnimal source)
		{
			MarkAsChild();
			Fetch(source);
		}
		
		private RazaAnimal(IDataReader reader)
		{
			MarkAsChild();
			Fetch(reader);
		}
		...
		public static RazaAnimal NewChild()
		{
			if (!CanAddObject())
		...
		internal static RazaAnimal GetChild(RazaAnimal source)
		{
			return new RazaAnimal(source);
		}
		
		internal static RazaAnimal GetChild(IDataReader reader)
		{
			return new RazaAnimal(reader);
		}
		
		public virtual RazaAnimalInfo GetInfo()
		{
			if (!CanGetObject())
				throw new System.Security.SecurityException(
				  moleQule.Resources.Messages.USER_NOT_ALLOWED);
			
			return new RazaAnimalInfo(this);
		}
		...
		/// </summary>
		/// <returns></returns>
		public override RazaAnimal Save()
		{
		...
		private void Fetch(RazaAnimal source)
		{
		...
		internal void Insert(RazaAnimales parent)
		{
		...
		internal void Update(RazaAnimales parent)
		{
			// if we're not dirty then don't update the database
			if (!this.IsDirty) return;
			...
			try
			{
				...
				RazaAnimalRecord obj = Session().Get<RazaAnimalRecord>(Oid);
		...
		internal void DeleteSelf(RazaAnimales parent)
		{
			// if we're not dirty then don't update the database
			if (!this.IsDirty) return;
			...
			{
				SessionCode = parent.SessionCode;
				Session().Delete(Session().Get<RazaAnimalRecord>(Oid));
			}
			catch (Exception ex)
			{
				iQExceptionHandler.TreatException(ex);
		...
		{
			string ra = nHManager.Instance.GetSQLTable(typeof(RazaAnimalRecord));
...
//				AND (RA.""DATE"" BETWEEN '" + conditions.FechaIniLabel + "' AND '" + conditions.FechaFinLabel + "')";
		...
		internal static string SELECT(QueryConditions conditions, bool lockTable)
		{
			...
			if (conditions != null)
			{
				query += ORDER(conditions.Orders, "RA", ForeignFields());
				query += LIMIT(conditions.PagingInfo);
			}

			query += Common.EntityBase.LOCK("RA", lockTable);
		...
		{
			return SELECT(new QueryConditions { RazaAnimal = RazaAnimalInfo.New(oid) }, lockTable);
		}
