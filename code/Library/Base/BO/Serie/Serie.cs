...
namespace moleQule.Library.Store
{
	...
	{	 
		...
		internal static string INNER(Library.Store.QueryConditions conditions)
		{
		...
		internal static string WHERE(Library.Store.QueryConditions conditions)
		{
			...
			query += Common.EntityBase.STATUS_LIST_CONDITION(conditions.Status, "S");

			if (conditions.Serie != null) 
				query += @"
					AND S.""OID"" = " + conditions.Serie.Oid;

			switch (conditions.TipoSerie)
			{
				...
				case ETipoSerie.Venta:
					query += @"
						AND (S.""TIPO"" IN (" + (long)conditions.TipoSerie + @"," + (long)ETipoSerie.CompraVenta + "))";
					break;

				default:
					query += @"
						AND S.""TIPO"" = " + (long)conditions.TipoSerie;
					break;
			}
        ...
        internal static string SELECT(Library.Store.QueryConditions conditions, bool lockTable)
        {
			...
			query += Common.EntityBase.LOCK("S", lockTable);
