...
{
	[Serializable()]
	public class ShippingCompanyMap : ClassMapping<ShippingCompanyRecord>
	{
		public ShippingCompanyMap()
		{
			Table("`STShippingCompany`");
			Lazy(true);

			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`STShippingCompany_OID_seq`" })); map.Column("`OID`"); });
			...
			Property(x => x.OidTarjetaAsociada, map => { map.Column("`OID_TARJETA_ASOCIADA`"); map.NotNullable(false); map.Length(32768); });
