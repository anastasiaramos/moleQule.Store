...
{
	[Serializable()]
	public class EmployeeMap : ClassMapping<EmployeeRecord>
	{
		public EmployeeMap()
		{
			Table("`STEmployee`");
			...
			Property(x => x.SueldoNeto, map => { map.Column("`SUELDO_NETO`"); map.NotNullable(false); map.Length(32768); });
			...
			Property(x => x.OidCrew, map => { map.Column("`OID_CREW`"); map.NotNullable(false); });
            Property(x => x.PayrollMethod, map => { map.Column("`PAYROLL_METHOD`"); map.NotNullable(false); });
		}
	}
}