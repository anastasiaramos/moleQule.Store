...
{
	[Serializable()]
	public class TransporterMap : ClassMapping<TransporterRecord>
	{
		public TransporterMap()
		{
			...
			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`STTransporter_OID_seq`" })); map.Column("`OID`"); });
			...
			Property(x => x.OidCuentaBAsociada, map => { map.Column("`OID_CUENTA_BANCARIA_ASOCIADA`"); });
