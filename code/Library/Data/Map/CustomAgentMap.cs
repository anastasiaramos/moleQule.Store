...
{
	[Serializable()]
	public class CustomAgentMap : ClassMapping<CustomAgentRecord>
	{
		public CustomAgentMap()
		{
			...
			Id(x => x.Oid, map => { map.Generator(Generators.Sequence, gmap => gmap.Params(new { sequence = "`STCustomAgent_OID_seq`" })); map.Column("`OID`"); });
