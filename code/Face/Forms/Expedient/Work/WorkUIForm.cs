...
{
    public partial class WorkUIForm : WorkForm
    {
		...
		public WorkUIForm(long oid)
            : this(oid, null) {}

        public WorkUIForm(long oid, Form parent) 
		...
		protected override void LoadCostes() { }
		...
		{
			/*Tools_NTB.Text = _work_reports.GetTotal(ETipoEntidad.Tool);
			Staff_NTB.Text = _work_reports.GetTotal(ETipoEntidad.Empleado);
			Materials_NTB.Text = _work_reports.GetTotal(ETipoEntidad.OutputDelivery);*/

			TotalWorkReports_NTB.Text = _work_reports.GetTotal().ToString("C2");
